...
public class WaveWriter {
	...
	public WaveWriter(String filename, int[][] data, long samplerate) {
		this.initWriter(filename, data, 0, data[0].length, samplerate);
	}

	public WaveWriter(String filename, int[][] data, int offset, int len, long samplerate) {
	...
	public static boolean saveSingleChannel(String filename, int[] data, long samplerate) {
		int[][] datar = new int[1][];
		datar[0] = data;
		WaveWriter writer = new WaveWriter(filename, datar, samplerate);
		writer.close();
		return true;
	}
}
