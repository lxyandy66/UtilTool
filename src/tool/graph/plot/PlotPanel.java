...
public class PlotPanel extends JPanel {
	...
	private static Color[] color={Color.BLACK,Color.RED,Color.DARK_GRAY,Color.GRAY,Color.LIGHT_GRAY,Color.CYAN,Color.BLUE};
	...
	private void commonInit() {
		...
		PlotPanelMouseListener mlst = new PlotPanelMouseListener(this);
		this.addMouseListener(mlst);
		this.addMouseMotionListener(mlst);

		this.setEnabled(true);
		this.setFocusable(true);
		PlotPanelKeyListener klst = new PlotPanelKeyListener(this);
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < drawableObjectStack.size(); ++i) {
			if (drawableObjectStack.get(i).show)
				this.paintDrawableObject(g, drawableObjectStack.get(i),i>=color.length?Color.RED:color[i]);
		}
		drawPlotInfo(g);
	}
...
//		g.setColor(drawableObject.attribute.lineColor);
	...
	private void paintDrawableObject(Graphics g, DrawableObject drawableObject,Color c){
		g.setColor(c);
		paintDrawableObject(g, drawableObject);
	} 
